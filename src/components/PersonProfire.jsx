...
import Email from './Icons/Wangyiyun'
...
const PersonProfire = memo(() => {
  ...
  useEffect(() => {
    const time =  setInterval(()=>{
      const color = "#" + (1048575 + Math.floor(Math.random() * 15728640)).toString(16);
      setrandmColor(color)
    },2000)
    return () => {
      clearInterval(time)
  ...
  return (
    <PersonProfireWapper>
      ...
      <div className="signature" style={{color:`${randmColor}`}}>满地都是六便士,他却抬头看月亮</div>
